...
import { Pencil, Eye, RefreshCw, Clock, Package, Truck, Plane, Ship, CheckCircle, Trash2, Plus, User } from 'lucide-react';
...
export const AdminOrdersManager: React.FC = () => {
  ...
  const handleCreateOrder = async () => {
    try {
      // First, check if user exists, if not create a profile
      const { data: existingProfile } = await supabase
        .from('profiles')
        .select('user_id')
        .eq('username', createFormData.customer_email)
        .single();

      let userId = existingProfile?.user_id;

      if (!existingProfile) {
        // Create a dummy user ID for non-registered customers
        userId = crypto.randomUUID();
        
        // Insert basic profile for non-registered customer
        const { error: profileError } = await supabase
          .from('profiles')
          .insert({
            user_id: userId,
            username: createFormData.customer_email,
            full_name: createFormData.customer_name,
            role: 'customer'
          });
      ...
      const { error } = await supabase
        ...
        .insert({
          ...
          item_name: createFormData.item_name,
          category: createFormData.category,
          total_weight: parseFloat(createFormData.total_weight),
          total_quantity: parseInt(createFormData.total_quantity),
          total_carton: parseInt(createFormData.total_carton),
          total_charge: parseFloat(createFormData.total_charge),
          shipping_method: createFormData.shipping_method,
          delivery_method: createFormData.delivery_method,
          street: createFormData.street,
          city: createFormData.city,
          district: createFormData.district,
          status: 'placed',
          special_notes: `Manually created by admin for ${createFormData.customer_name}`
        });
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="bg-slate-900 border-white/10 max-w-4xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-gray-300">
              Create a new order for any customer (registered or non-registered)
            </DialogDescription>
          </DialogHeader>
          <div className="grid grid-cols-2 gap-4">
            <div>
            ...
            <div>
              ...
              <Input
                type="number"
                value={createFormData.total_weight}
                onChange={(e) => setCreateFormData({ ...createFormData, total_weight: e.target.value })}
                className="bg-black/20 border-white/10 text-white"
              />
            </div>
            <div>
              ...
              <Input
                type="number"
                value={createFormData.total_quantity}
                onChange={(e) => setCreateFormData({ ...createFormData, total_quantity: e.target.value })}
                className="bg-black/20 border-white/10 text-white"
              />
            </div>
            <div>
              ...
              <Input
                type="number"
                value={createFormData.total_carton}
                onChange={(e) => setCreateFormData({ ...createFormData, total_carton: e.target.value })}
                className="bg-black/20 border-white/10 text-white"
              />
            </div>
            <div>
              ...
              <Input
                type="number"
                value={createFormData.total_charge}
                onChange={(e) => setCreateFormData({ ...createFormData, total_charge: e.target.value })}
                className="bg-black/20 border-white/10 text-white"
              />
            </div>
            <div>
              <Label className="text-white">Shipping Method</Label>
              <Select
                value={createFormData.shipping_method}
                onValueChange={(value) => setCreateFormData({ ...createFormData, shipping_method: value })}
              >
                <SelectTrigger className="bg-black/20 border-white/10 text-white">
                  <SelectValue />
                </SelectTrigger>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => setCreateFormData({ ...createFormData, delivery_method: value })}
              >
                <SelectTrigger className="bg-black/20 border-white/10 text-white">
                  <SelectValue />
                </SelectTrigger>
